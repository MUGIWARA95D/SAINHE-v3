# SAINHE — Bloc-notes de projet
> Mis à jour automatiquement par Claude. Dis "note :" pour ajouter une idée.
...
- ~~**IA texte** : Gemini 2.5 Flash~~ → **ABANDONNÉ** — remplacé par scraping RSS + templates Jinja2
- **Fetch prix/volumes** : `yfinance` lib Python — simple, fiable, maintenu (Option 1A)
- **Stack** : Python + Jinja2 + Chart.js (CDN) — page HTML statique
- **Architecture** : Cache-first — HTML généré une fois, lu par tous les users
- **Hosting** : Local d'abord → Cloudflare Pages + domaine SAINHE.com ensuite
...
- **Source macro** : bls.gov + federalreserve.gov + marketwatch.com (HTML statique)
...
### Fichiers créés
```
SAINHE/sainhe-newsletter/
├── .env                  ✅ clé Gemini (sécurisée)
├── .gitignore            ✅
├── config.json           ✅ watchlist + secteurs
├── requirements.txt      ✅
├── run.py                ✅ point d'entrée local
├── demo.py               ✅ génère page avec données figées
├── scripts/
│   ├── fetcher.py        ✅ yfinance (marché + secteurs + watchlist)
│   ├── ai_writer.py      ✅ Gemini API (cache 12h)
│   └── page_builder.py   ✅ Jinja2 → HTML
├── templates/
│   └── dashboard.html    ✅ design dark actuel (à refaire en crème)
└── output/
    └── index.html        ✅ page démo générée
```

### Problèmes connus
- yfinance bloqué dans le VM Cowork (proxy 403) → fonctionne sur machine locale
- FMP crédits épuisés dans la session actuelle
- Gemini API bloquée dans le VM → fonctionne sur machine locale
- Template actuel trop "futuriste" → refonte design en cours

### Prochaines étapes
- [ ] Refonte complète du template HTML (crème/beige Apple-style)
- [ ] Ajouter sélecteur de langue (7 langues)
- [ ] Ajouter dark mode toggle
- [ ] Coder le fil d'actualité incrémental
- [ ] Automatiser le planning via scheduled tasks
- [ ] Hugo valide la liste complète des rubriques de la page
