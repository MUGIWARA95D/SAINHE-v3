...
# ------------------------------------------------------------------ Net Debt ASC 842
def financial_debt(cd: CompanyData, fy: int | None = None):
    """Dette financière totale = LT + ST + FinanceLeaseLiability (.txt NOTE ASC 842).
    Si un tag retenu inclut DÉJÀ les leases (LongTermDebtAndCapitalLeaseObligations…,
    taxonomie FASB 2024+ : KO p.ex.), FinanceLeaseLiability n'est PAS recompté.
    OperatingLeaseLiability est EXCLU (obligation opérationnelle, pas dette financière).
    Retourne None si ni dette LT ni dette ST n'est disponible.
    """
    lt_dp, st_dp = cd.point("lt_debt", fy), cd.point("st_debt", fy)
    if lt_dp is None and st_dp is None:
        return None
    tags = (lt_dp.tag if lt_dp else "") + "+" + (st_dp.tag if st_dp else "")
    leases_included = "CapitalLeaseObligations" in tags or "FinanceLease" in tags
    fl = 0.0 if leases_included else _nz(cd.value("finance_lease_liab", fy))
    return _nz(lt_dp and lt_dp.value) + _nz(st_dp and st_dp.value) + fl


def net_debt(cd: CompanyData, fy: int | None = None):
    """Net Debt = dette financière totale − Cash (.txt NOTE ASC 842)."""
    fd = financial_debt(cd, fy)
    if fd is None:
        return None
    return fd - _nz(cd.value("cash", fy))
